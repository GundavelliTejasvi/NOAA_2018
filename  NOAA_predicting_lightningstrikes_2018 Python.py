...
# ## Overview
# 
#  I will use pandas to examine 2018 lightning strike data collected by the National Oceanic and Atmospheric Administration (NOAA). Then, we will calculate the total number of strikes for each month and plot this information on a bar graph.
# To predict future lightning strikes
# In[1]:
...
# The `date` column is an 'object' type rather than a 'date' type. Objects are strings. When dates are encoded as strings, they cannot be manipulated as easily. Converting string dates to datetime will enable you to work with them much more easily.
...
# A common mistake among is using `count()` instead of `sum()`, and vice versa. In this case, `count()` would return the number of occurrences of each date in the dataset, which is not the desired result.
...
# To help read the data more easily, convert the month number to text using the datetime function `dt.month_name()` and add this as a new column in the dataframe. `str.slice` will omit the text after the first three letters. 
...
# The objective is to plot the total number of strikes per month as a bar graph. To help with the plotting, I will create a new dataframe called `df_by_month`. This will allow to easily access the month, month text, and total number of strikes for each month. 
...
#  Pyplot's `plt.bar()` function takes positional arguments of `x` and `height`, representing the data used for the x- and y- axes, respectively. The x-axis will represent months, and the y-axis will represent strike count.
...
# ## Conclusion
# August 2018 had the most lightning strikes, while November and December 2018 had the least amount of strikes.
